...
def build(target, options, work_dir, out_dir, env):
	...
	subprocess.check_call([gn, 'gen', out_dir, '--args=' + ' '.join(gn_args).lower()], cwd=work_dir, env=env, shell=False)
	subprocess.check_call([ninja, '-C', out_dir, target], cwd=work_dir, env=env, shell=False)
